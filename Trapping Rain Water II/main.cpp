...
public:
    ...
    static int trapRainWater(vector<vector<int>>& height) {
        ...
        boundary.reserve(2 * (m + n));

        // Add boundary cells and mark visited
        for (int i = 0; i < m; i++) {
            boundary.push_back(pack(height[i][0], i, 0));
            boundary.push_back(pack(height[i][n - 1], i, n - 1));
            height[i][0] = height[i][n - 1] = -1; // visited
        }

        for (int j = 0; j < n; j++) {
            boundary.push_back(pack(height[0][j], 0, j));
            boundary.push_back(pack(height[m - 1][j], m - 1, j));
